...
public class NewWomanMove : MonoBehaviour
{
    public static NewWomanMove instance { get; private set; }

    public NewWomanCtrl womanctrl;
    ...
    private void Update()
    {
        if (SceneManager.GetActiveScene().name.Equals("BusanMapMorning") ||
                ...
                mpu_value[2] >= -32768f && mpu_value[2] <= 32768)
            {
                ...
                if (connetState && Busan_UIManager.instance.gameStartState.Equals(true))
                {
                    if ((Mathf.Abs(gapX) < 0.002f && Mathf.Abs(gapY) < 0.002f && 
                        Mathf.Abs(gapZ) < 0.002f && stop == false))
                    {
                        if (Busan_UIManager.instance.speed < 0f)
                        {
                            ...
                            {
                                speed = 0f;
                                this.womanctrl.Animator_Speed(false, false, 1);
                            }
                            else if (speed > 0f)
                            {
                                speed -= 0.05f;
                                this.womanctrl.Animator_Speed(true, false, speed);
                            }
                            else if (speed == 0f)
                            {
                                speed = 0f;
    ...
    void PlaySpeed(float _standardSpeed, float _bikeSpeed, float _animSpeed, float _maxSpeed, bool _normalState, bool _speedState)
    {
        ...
        this.womanctrl.Animator_Speed(_normalState, _speedState, speed);
    ...
    public float BikeMaxSpeed(float _standardSpeed)
    {
        if (PlayerPrefs.GetString("Busan_UseItemSpeed").Equals("Speed"))
        {
            if (Busan_UIManager.instance.mountainUpstate.Equals(true))   //오르막길
                ...
                _standardSpeed = (_standardSpeed) * 1.3f * 1.4f;
