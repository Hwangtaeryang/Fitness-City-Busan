...
public class StoreBuyPopupCtrl : MonoBehaviour
{
    ...
    {
        titleText.text = PlayerPrefs.GetString("Busan_StoreBuyTitle") + " 스타일";
        myGold_i = int.Parse(myGold.text.Replace(",",""));
        price_i = PlayerPrefs.GetInt("Busan_StoreBuy_Price");
        folderName_s = PlayerPrefs.GetString("Busan_StoreBuy_FolderName");
        imgName_s = PlayerPrefs.GetString("Busan_StoreBuy_ImageName");
    ...
    public void BuySaveButtonOn()
    {
        ...
        if (price_i <= myGold_i)
        {
            ...
            if(myGold_i != 0)
            {
                ...
                PlayerPrefs.SetInt("Busan_Player_Gold", myGold_i);
            }
            else if(myGold_i.Equals(0))
            {
                myGold.text = "0";  //가격 텍스트에 뿌리기
                PlayerPrefs.SetInt("Busan_Player_Gold", 0);
            }

            if (imgName_s == "ExpUp")
            {
                int amount = PlayerPrefs.GetInt("Busan_ExpUpAmount");
                amount += 1;
                PlayerPrefs.SetInt("Busan_ExpUpAmount", amount);  //갯수를 추가해준다.

                if (PlayerPrefs.GetString("Busan_ItemPurchase").Equals("MissionOk"))
                    PlayerPrefs.SetString("Busan_ItemPurchase", "MissionOk");   //아이템 구매여부
                else
                    PlayerPrefs.SetString("Busan_ItemPurchase", "Yes");   //아이템 구매여부
            }
            ...
            {
                int amount = PlayerPrefs.GetInt("Busan_CoinUpAmount");
                amount += 1;
                PlayerPrefs.SetInt("Busan_CoinUpAmount", amount);  //갯수를 추가해준다.
            ...
            {
                int amount = PlayerPrefs.GetInt("Busan_SpeedUpAmount");
                amount += 1;
                PlayerPrefs.SetInt("Busan_SpeedUpAmount", amount);  //갯수를 추가해준다.
            ...
            if(PlayerPrefs.GetString("Busan_GoldUse").Equals("MissionOk"))
                PlayerPrefs.SetString("Busan_GoldUse", "MissionOk");    //코인 사용 여부
            else
                PlayerPrefs.SetString("Busan_GoldUse", "Yes");    //코인 사용 여부
    ...
    {
        int level = PlayerPrefs.GetInt("Busan_Player_Level");
        int takeExp = _takeExp;
        int currExp = PlayerPrefs.GetInt("Busan_Player_CurrExp");
        ...
        PlayerPrefs.SetInt("Busan_Player_Level", level);
        PlayerPrefs.SetInt("Busan_Player_TakeExp", takeExp);
        PlayerPrefs.SetInt("Busan_Player_CurrExp", currExp);
